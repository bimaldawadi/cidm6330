...
from bookmarksDomainModel import bookmarksDomainModel
import sys

import requests

from dataAccess import DatabaseManager
...
class CreateBookmarksTableCommand:
    ...
    def execute(self):
        db.create_table(
            "bookmarks",
            {
                "id": bookmarksDomainModel.id,
                "title": bookmarksDomainModel.title,
                "url": bookmarksDomainModel.url,
                "notes": bookmarksDomainModel.notes,
                "date_added": bookmarksDomainModel.date_added,
            },
        )
...
class AddBookmarkCommand:
    ...
    def execute(self, data, timestamp=None):
        data[bookmarksDomainModel.date_added] = datetime.utcnow().isoformat()
        db.add(bookmarksDomainModel, data)
...
class DeleteBookmarkCommand:
    ...
    def execute(self, data):
        db.delete(bookmarksDomainModel, {bookmarksDomainModel.id: data})
...
class ImportGitHubStarsCommand:
    ...
    def _extract_bookmark_info(self, repo):
        return {
            "title": repo[bookmarksDomainModel.title],
            "url": repo[bookmarksDomainModel.url],
            "notes": repo[bookmarksDomainModel.notes],
        }
...
class EditBookmarkCommand:
    def execute(self, data):
        db.update(
            bookmarksDomainModel,
            {"id": data[bookmarksDomainModel.id]},
            data["update"],
        )
